 ...
 *
 */
public class ChatSDKReceiverParse extends BroadcastReceiver {

    private static final String TAG = ChatSDKReceiverParse.class.getSimpleName();
    ...
    private void createMessageNotification(final Context context, Intent intent, String channel){
        try {
            ...
            String entityID = json.getString(PushUtilsParse.MESSAGE_ENTITY_ID);
            final String threadEntityID = json.getString(PushUtilsParse.THREAD_ENTITY_ID);
            final String senderEntityId = json.getString(PushUtilsParse.MESSAGE_SENDER_ENTITY_ID);
            ...
            final Long dateLong =json.getLong(PushUtilsParse.MESSAGE_DATE);
            final Date date = new Date(dateLong);
            final Integer type = json.getInt(PushUtilsParse.MESSAGE_TYPE);
            final String messagePayload = (json.getString(PushUtilsParse.MESSAGE_PAYLOAD));
            ...
            try {
                NotificationUtils.createAlertNotification(context, BDefines.MESSAGE_NOTIFICATION_ID, resultIntent,
                        NotificationUtils.getDataBundle(context.getString(R.string.not_message_title),
                                context.getString(R.string.not_message_ticker), json.getString(PushUtilsParse.CONTENT)));
            } catch (JSONException e) {
                if (DEBUG) Timber.e(e.getCause(), "JSONException: %s", e.getMessage());
            }
        }
        ...
        try {
            ...
            NotificationUtils.createAlertNotification(context, BDefines.FOLLOWER_NOTIFICATION_ID, resultIntent,
                    NotificationUtils.getDataBundle(context.getString(R.string.not_follower_title), context.getString(R.string.not_follower_ticker),
                            json.getString(PushUtilsParse.CONTENT)));
        } catch (JSONException e) {
            if (DEBUG) Timber.e(e.getCause(), "JSONException: %s", e.getMessage());
        }
    }
